...
| GitHub Actions | 安全工作流已写入本地并通过 YAML 检查；尚未上传或实际触发 |
| GitHub 登录 | 本机现有 GitHub CLI 凭证失效，需要重新完成官方网页授权 |
| 止损成交规则 | v0.1 草案，尚未批准 |
| 模拟成交引擎、持续风险监控 | 尚未实现 |
| 实盘下单 | 未实现、未启用、未下单 |
...
分享时使用空白凭证模板，不复制真实 env、编辑器交换文件或备份。GitHub 密钥只能填写在仓库 Actions Secrets 中；不要写入工作流、文档或聊天。Git 忽略规则不会自动过滤 Finder 复制或压缩包。
