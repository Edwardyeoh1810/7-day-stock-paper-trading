...
- Latest `data/journal/` record, if present
...
Run `python3 -B scripts/run_observation.py` from the project for a due slot, or use `--manual` for an explicit check. The process lock serializes runs; SQLite uses unique date/slot IDs and prevents a new run while an earlier run is unfinished. `--recover` acquires the same lock, restores completed exports and marks interrupted read-only executions. It never retries orders or rolls portfolio values backwards.

API-check journals and evidence are deterministic exports from `data/runs.sqlite3`; do not manually edit them. Put research in separate files with a `-research` suffix. `data/current-state.json` remains the paper portfolio record; no trade/fill ledger has been implemented. Latest run metadata can be recovered from the database.
