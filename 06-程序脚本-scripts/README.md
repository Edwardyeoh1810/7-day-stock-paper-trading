...
- `binance_readiness_check.py`：只读检查账户、权限、交易规则、报价和未结订单；`BINANCE_ENV=demo` 时检查 Binance 模拟盘现货。
- `run_observation.py`：识别六个日程时点，写入 SQLite、日志和证据。
- `paper_engine.py`：执行 paper 决策；通过 `demo_orders.py` 只向 Binance 模拟盘下单，不包含正式环境下单路径。
- `demo_orders.py`：唯一的 POST 路径；固定指向 `demo-api.binance.com`（虚拟资金），`BINANCE_ENV` 不是 `demo` 时拒绝运行。
