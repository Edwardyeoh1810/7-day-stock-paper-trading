...
All 42 times and bilingual schedules are checked offline by the test suite. The demo readiness check, market snapshots for all five pairs, validate-only test orders, a no-trade engine pass, and a live demo round trip (market buy, OCO placement, status, cancel, market sell) were verified on 2026-09-21. Scheduled tasks only run while the Claude desktop app is open; every run must compare the actual host time with the intended slot and must never backfill a historical trade.
