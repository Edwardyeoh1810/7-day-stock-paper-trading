...
4. Update `reviews/LESSONS.md`: **at most 10 lessons of at most 100 words each**, one paragraph per lesson stating the rule, the evidence and the sample size, the most useful first; no amendment histories (the daily reviews hold those). Add a lesson only when at least 3 trades or 5 decisions support it. A lesson may make future decisions more selective (skip a setup, demand an extra evidence category, avoid a time of day); it may never loosen or override a rule or a risk limit in `AGENTS.md`, the strategy, `readiness.json` or `paper-config.json`. **The review must retire or narrow a lesson** when the data no longer supports it, when it is contradicted, or when it would exclude every watchlist contract or block entries for more than three consecutive days: lessons are hypotheses, and a lesson that shuts the experiment is a failed one. Record each retirement in the daily review with the reason. With fewer than 30 closed trades, phrase lessons as hypotheses.
...
The review changes how carefully the AI decides, not what it is allowed to do. It may add, narrow and retire its own lessons; it may not change a rule. Risk limits, leverage, watchlist, schedule and code change only when Edward approves a proposal. A small sample proves nothing: a profitable week is not evidence of a profitable system, and the review must say so.
