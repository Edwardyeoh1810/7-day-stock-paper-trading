...
- `09-API密钥-仅本地/binance-api.env`: dedicated local-only Binance API credential file with empty placeholders.
...
- `.gitignore`: ignores `.env`, `.env.*`, and `09-API密钥-仅本地/binance-api.env`, while keeping example env files trackable.
...
- Signed account check:
  - Skipped because `BINANCE_API_KEY` and `BINANCE_API_SECRET` are empty in local `09-API密钥-仅本地/binance-api.env`.
