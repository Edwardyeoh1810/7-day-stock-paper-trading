...
class RuntimeTests(unittest.TestCase):
    ...
    def test_bilingual_schedule_matches(self):
        en = json.loads((ROOT / "routines/schedule.json").read_text())
        zh = json.loads((ROOT / "routines/schedule.zh-CN.json").read_text())
    ...
    def test_export_recovery_preserves_portfolio(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            for name in ("data/journal", "data/evidence"):
                (root / name).mkdir(parents=True)
            state_path = root / "data/current-state.json"
            ...
            self.assertEqual(len(list((root / "data/journal").glob("*.md"))), 1)
