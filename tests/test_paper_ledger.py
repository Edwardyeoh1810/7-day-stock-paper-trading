...
class PaperLedgerTests(unittest.TestCase):
    ...
    def _write_all(self):
        for path, payload in (
            (self.root / "data/current-state.json", self.state),
            (self.root / "state/readiness.json", self.readiness),
            (self.root / "state/paper-config.json", self.config),
            (self.root / "data/paper-ledger.json", self.ledger_data),
        ):
            path.write_text(json.dumps(payload), encoding="utf-8")

    def make_ledger(self):
        return PaperLedger(
            self.root / "data/current-state.json",
            self.root / "state/readiness.json",
            self.root / "state/paper-config.json",
            self.root / "data/paper-ledger.json",
        )
    ...
    def test_open_respects_position_and_trade_limits(self):
        ...
        state = json.loads((self.root / "data/current-state.json").read_text())
    ...
    def test_reconcile_restores_last_event_snapshot(self):
        self.make_ledger().open_long(self.request(), self.now)
        damaged = json.loads((self.root / "data/current-state.json").read_text())
        damaged["cash_usdt"] = 1
        (self.root / "data/current-state.json").write_text(json.dumps(damaged), encoding="utf-8")
