...
STARTUP = ("AGENTS.md", "AGENTS.zh-CN.md", "docs/TRADING-STRATEGY.md", "docs/TRADING-STRATEGY.zh-CN.md",
           "routines/schedule.json", "routines/schedule.zh-CN.json", "routines/CONTINUITY.md",
           "routines/CONTINUITY.zh-CN.md", "state/readiness.json", "data/current-state.json")
...
def export_records(store):
    ...
    for run_id, payload in records:
        atomic_write(ROOT / "data" / "journal" / (run_id + ".md"), payload["journal"])
        atomic_write(ROOT / "data" / "evidence" / (run_id + ".json"), json.dumps(payload["check"], indent=2) + "\n")
    # Portfolio JSON remains authoritative; run recovery never rolls it backwards.
    if records:
        run_id, payload = records[-1]
        path = ROOT / "data" / "current-state.json"
...
def main():
    ...
    schedule = json.loads((ROOT / "routines/schedule.json").read_text())
    ...
    if not args.manual and not args.recover and slot is None:
        ...
        return 0
    with (ROOT / "state" / "run.lock").open("a") as lock:
        try:
        ...
        try:
            ...
            journals = sorted((ROOT / "data" / "journal").glob("*.md"), key=lambda p: p.stat().st_mtime)
            if journals:
            ...
            if not store.begin(run_id):
                ...
                return 0
            state = json.loads((ROOT / "data" / "current-state.json").read_text())
