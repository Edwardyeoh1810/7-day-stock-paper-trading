...
def build_ledger():
    return PaperLedger(
        ROOT / "data" / "current-state.json",
        ROOT / "state" / "readiness.json",
        ROOT / "state" / "paper-config.json",
        ROOT / "data" / "paper-ledger.json",
    )
...
def write_records(run_id, decision, result, now):
    ...
    atomic_json(ROOT / "data" / "current-state.json", state)
    evidence = {
        "run_id": run_id,
        "timestamp": now.isoformat(),
    ...
    atomic_json(ROOT / "data" / "evidence" / (run_id + ".json"), evidence)
    journal_path = ROOT / "data" / "journal" / (local.date().isoformat() + ".md")
    action = decision["action"]
    ...
    lines = [
        ...
        "- Evidence captured: `data/evidence/" + run_id + ".json`.",
